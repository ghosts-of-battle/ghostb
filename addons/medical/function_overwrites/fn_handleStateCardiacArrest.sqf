...
if (_timeDiff >= 1) then {
    ...
    private _totalTimeLeft = _unit getVariable ["GHOSTB_REVIVAL", GHOSTB_medical_persistantReviveTimer];
    if(GHOSTB_medical_persistantRevive) then {
    ...
    _unit setVariable ["GHOSTB_REVIVAL", _totalTimeLeft, true];
};
