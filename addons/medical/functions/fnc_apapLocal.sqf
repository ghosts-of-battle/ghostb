...
if (_adjustments isNotEqualTo []) then {
     ...
     private _nillStrength = 0;
    {
        ...
        if(_medication == "GHOSTB_Apap") then {
        ...
        if(_medication == "GHOSTB_Apap_half") then {
            _halfStrength = _halfStrength + 1;
        };
        if(_medication == "GHOSTB_Apap_nil") then {
            _nillStrength = _nillStrength + 1;
        };
    } forEach _adjustments;
    switch (true) do {
        case (_nillStrength > 1):{
            _changedMedication set [0, "GHOSTB_Apap_nil"];
        ...
        case (_fullStrength > 2):{
            _changedMedication set [0, "GHOSTB_Apap_half"];
